...
def send_email(to_addr, message, subject='DLAIE AUTOGRADER: Your results', from_addr='mcskwayrd@gmail.com', valid_emails=[]):
...
def update_and_run_nb(row, assignment_dir="assigment_N", noemail=False, valid_emails=[], force_execute=False):
    ...
    dst_file = f"{assignment_dir}/{student_id}_{name.replace(' ','_')}.ipynb"
...
if __name__=="__main__":
    ...
    ss_url = ss_sharing_url_to_csv(ss_url)
    print(f"   CSV sharing url = {ss_url}")
    download_if_newer_gdrive(ss_url, ss_file)  # download spreadsheet from Google Drive (if newer than local copy)

    ## Read in local version of Google Forms spreadsheet
    ...
    df.apply(update_and_run_nb, axis=1,  assignment_dir=assignment_dir, noemail=args.noemail, valid_emails=valid_emails)
